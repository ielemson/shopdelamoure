// Delamoure: force light mode globally
...
(function (e) {
  ...
  APP.Others = {
    ...
    canvasCart: function () {
      e(".sshop-down").on("click", function (s) {
        s.preventDefault();
        var o = e(this).parent(".input-group"),
            n = o.find("input:not([disabled])"),
            t = parseInt(n.val());
        t > 0 && (t -= 1, n.val(t));
      }), e(".sshop-up").on("click", function (s) {
        s.preventDefault();
        var o = e(this).parent(".input-group"),
            n = o.find("input:not([disabled])"),
            t = n.val();
  ...
  APP.colorModes = {
    init: function () {
      // Delamoure uses light mode only.
      document.documentElement.setAttribute("data-bs-theme", "light");

      // Remove any previously saved dark/auto preference.
      try {
        localStorage.removeItem("theme");
      } catch (error) {
        // Ignore storage access errors.
  ...
  // Apply immediately so the page never switches to dark mode.
  APP.colorModes.init();
